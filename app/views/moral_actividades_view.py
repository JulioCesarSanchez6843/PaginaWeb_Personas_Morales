# app/views/moral_actividades_view.py
...
# Listar (y filtrar) asignaciones con texto libre para nombres
@moral_actividades_view.route('/', methods=['GET'])
def listado():
    filtros = {
        'nombre_moral':     request.args.get('nombre_moral', '').strip(),
        'nombre_actividad': request.args.get('nombre_actividad', '').strip()
    }

    where_clauses = []
    params = {}

    if filtros['nombre_moral']:
        where_clauses.append("UPPER(pm.RAZON_SOCIAL) LIKE :nombre_moral")
        params['nombre_moral'] = f"%{filtros['nombre_moral'].upper()}%"
    if filtros['nombre_actividad']:
        where_clauses.append("UPPER(ae.NOMBRE) LIKE :nombre_actividad")
        params['nombre_actividad'] = f"%{filtros['nombre_actividad'].upper()}%"
